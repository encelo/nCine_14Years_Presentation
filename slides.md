...
<figcaption>My presentation on a very broken projector</figcaption>
</figure>
</div>
...
|-------------------------------------------------------------------------------|
|C++                            378          17113           3298          83872|
|C/C++ Header                   343           7422           4277          30645|
...
|INI                              1              2              0             10|
|-------------------------------------------------------------------------------|
|SUM:                           809          25652           7995         122894|
\-------------------------------------------------------------------------------/
```

<figcaption>
The main nCine repository, counting only the <code>master</code> branch and excluding external projects.<br/>
Over 25,000 lines are dedicated to unit tests. 😱
</figcaption>
</figure>
...
- Stylistic coherence with [Artistic Style](https://astyle.sourceforge.net/) (<carbon-branch /> `codestyle`, <carbon-commit /> [d111d9c1](https://github.com/nCine/nCine/commit/d111d9c1), 🗓️ Oct - Dec)
  - Moved private headers in `src/include` so the user only sees the public interface
...
- I moved to Stockholm to work as a Rendering Engineer for Frostbite 🇸🇪 (🗓️ Mar)
  - During my interview: "I don't know how STL does it, but in my framework..." 😎
  - Shortly after, I was ready to release the sources, but contract restrictions prevented it
  - Still distributing the binary release to friends on Discord
  - Credited in EA games like FIFA 18-20, Battlefront II, Battlefield V, Anthem, and more
...
- Add a `RenderBufferManager` to use a single VBO, IBO, and UBO for all scene data
  ...
  - 📜 Reference: [Buffer Obejct Streaming](https://www.khronos.org/opengl/wiki/Buffer_Object_Streaming) OpenGL Wiki page
...
SFINAE is used to restrict function template overloads based on type traits (before C++20 concepts).<br/>
...
<figcaption class="left">
Object destruction and array deallocation are two separate operations.<br/>
SFINAE is used by <code>destructArray()</code> to decide whether to invoke the destructor for <code>T</code>.<br/>
...
<figcaption>
<a href="https://github.com/nCine/ncParticleEditor/"><strong>ncParticleEditor</strong></a> showing a project by Helba
</figcaption>
</figure>
...
- I left Sweden and EA behind for Spain 🇪🇸 (🗓️ Feb)
...
- Projects upload a build artifact in the `projectName-artifacts` repository
  - This special repository has a branch per each platform
...
- I wrote a retrospective [article](https://encelo.github.io/2021-06-21-ten-years-ncine/) about the first ten years and got interviewed in a [podcast](https://runtimepodcast.com/#5)
...
- Explore _new_ and ambitious projects:
  ...
  - Build a fully-fledged ImGui editor with a runtime "player" (like Unity/Godot)
